...
app.('', async (req, res) => {
  try {
    ...
    const stmt = await db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value');
...
app.('', async (req, res) => {
  try {
    const stores = await db.prepare('SELECT * FROM stores ORDER BY id DESC').all();
    res.json(stores);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.('', async (req, res) => {
  try {
    const { name, phone, address, lat, lng, contact_person } = req.body;
    if (!name || lat == null || lng == null) {
      return res.status(400).json({ error: 'Do\'kon nomi va lokatsiyasini ko\'rsating' });
    }
    const stmt = await db.prepare(`
      INSERT INTO stores (name, phone, address, lat, lng, contact_person)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    const result = stmt.run(name.trim(), phone || '', address || '', Number(lat), Number(lng), contact_person || '');
...
app.('', async (req, res) => {
  try {
    ...
    await db.prepare(`
      UPDATE stores 
      SET name = ?, phone = ?, address = ?, lat = ?, lng = ?, contact_person = ?
      WHERE id = ?
    `).run(name.trim(), phone || '', address || '', Number(lat), Number(lng), contact_person || '', id);
...
app.('', async (req, res) => {
  try {
    ...
    const checkStmt = await db.prepare('SELECT id FROM deliveries WHERE store_id = ? AND date = ?');
    ...
    let addedCount = 0;
    store_ids.forEach((store_id, idx) => {
      const exists = checkStmt.get(store_id, targetDate);
      if (!exists) {
        insertStmt.run(store_id, targetDate, idx);
        addedCount++;
      }
    });
...
app.('', async (req, res) => {
  try {
    ...
    if (Array.isArray(ordered_ids)) {
      const updateStmt = await db.prepare('UPDATE deliveries SET order_index = ? WHERE id = ?');
...
app.('', async (req, res) => {
  try {
    ...
    await db.prepare(`
      UPDATE driver_trips
      SET total_km = ?, gas_spent_sum = ?, last_lat = ?, last_lng = ?, updated_at = datetime('now', 'localtime')
      WHERE id = ?
...
app.('', async (req, res) => {
  try {
    ...
    if (!trip) {
      ...
      await db.prepare("UPDATE driver_trips SET total_km = ?, gas_spent_sum = ?, updated_at = datetime('now', 'localtime') WHERE id = ?").run(km, gasSpent, trip.id);
    }
...
app.('', async (req, res) => {
  try {
    const sales = await db.prepare('SELECT * FROM sales ORDER BY id DESC').all();
    res.json(sales);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.('', async (req, res) => {
  try {
    ...
    const result = stmt.run(saleDate, buyer_name.trim(), material_type, w, p, disc, total, payment_type || 'NAQD', notes || '');
...
app.('', async (req, res) => {
  try {
    const expenses = await db.prepare('SELECT * FROM expenses ORDER BY id DESC').all();
    res.json(expenses);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.('', async (req, res) => {
  try {
    // 1. Sotib olingan xomashyolar (Deliveries 'BAJARILDI')
    ...
    const saleSalafan = await db.prepare(`
      SELECT 
        ...
        COALESCE(SUM(total_amount), 0) as total_sold_sum
      FROM sales
      WHERE material_type = 'SALAFAN'
    `).get();

    const totalSoldSum = saleKarton.total_sold_sum + saleSalafan.total_sold_sum;

    // 3. Ombor qoldig'i (kg)
    const kartonStockKg = Math.max(0, buyStats.total_karton_bought_kg - saleKarton.total_sold_kg);
    const salafanStockKg = Math.max(0, buyStats.total_salafan_bought_kg - saleSalafan.total_sold_kg);

    // 4. Xarajatlar (Gaz va Boshqa)
    const gasExpense = await db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE category = 'GAZ'").get().total;
    const otherExpense = await db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE category = 'BOSHQA'").get().total;
    const totalExpenses = gasExpense + otherExpense;

    // 5. Sof foyda = Jami sotuv summasi - Jami xarid summasi - Xarajatlar
